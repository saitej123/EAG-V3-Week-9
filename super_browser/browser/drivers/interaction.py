"""Browser drivers — a11y (text LLM) and set-of-marks vision (Gemini VLM)."""
...
from .elements import PageSnapshot, enumerate_interactives
from .gemini_client import GeminiClient
from .marks import annotate, to_data_url
        ...
        "actions": {
            ...
            "items": {
                ...
                "properties": {
                    "type": {
                        "type": "string",
                        "enum": ["click", "type", "key", "scroll", "drag", "navigate", "wait", "done"],
                    },
                    "mark": {"type": "integer"},
                    "value": {"type": "string"},
                    "clear": {"type": "boolean"},
...
SYSTEM_PROMPT_VISION = (
    ...
    "  drag(from_x, from_y, to_x, to_y) — mouse drag in CSS pixels\n"
    "  navigate(value)            — go to the URL in `value`\n"
    "  wait(seconds)              — pause to let the page settle\n"
    "  done(success, note)        — finish; success=true if the goal is met\n"
    "CRITICAL: If the goal requires gathering data from multiple pages (like opening 3 repositories), "
    "you MUST perform the actions (click, scroll, etc.) to visit those pages. "
    "DO NOT just return `done` immediately. You must navigate and explore until you have all the required information.\n"
...
SYSTEM_PROMPT_A11Y = (
    ...
    "  - If the goal requires gathering data from multiple pages (like opening 3 repositories), "
    "    you MUST perform the actions (click, scroll, etc.) to visit those pages. "
    "    DO NOT just return `done` immediately. You must navigate and explore until you have all the required information.\n"
...
async def _dispatch(action: dict, page: Page, snap: PageSnapshot) -> str:
    ...
    if t == "navigate":
        ...
        if not url:
            return "error: missing url for navigate"
...
class BaseDriver:
    ...
    def __init__(self, page: Page, client: GeminiClient, config: DriverConfig):
        self.page = page
        self.client = client
        self.config = config
    ...
    def _history_text(self) -> str:
        ...
        for s in recent:
            acts = ", ".join(
                f"{a['type']}({a.get('mark') or a.get('value', '')})" for a in s.actions[:3]
            )
    ...
    async def step(self, turn: int) -> tuple[bool, bool, str]:
        ...
        actions = list(parsed.get("actions") or [])[:2]
        outcomes: list[str] = []
    ...
    async def run(self) -> DriverResult:
        ...
        # If we reached the step cap but we have some steps, it's a partial success
